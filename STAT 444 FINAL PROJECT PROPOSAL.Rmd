...
* The distribution of Sale Price looks very right-skewed with
most values clustered around the left tail and a significantly longer right tail, signifying extreme high values
...
```{r, echo=FALSE, fig.cap="Histogram of Sale Price\\label{}"}
options(scipen=10)
...
```{r, echo=FALSE, fig.cap="Histogram of Log Sale Price\\label{}"}
...
We also assessed some of the non-numeric variables in the dataset and discovered some interesting results.
* Neighborhood
   + There appears to be a significant difference in the average property sale price from neighborhood to neighborhood.
* The dataset contains a good number of categorical variables with little variability in distribution and likely holds no predictive power
